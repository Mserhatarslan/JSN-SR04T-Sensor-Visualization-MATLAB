...
void setup() {
  ...
  pinMode(echoPin1,INPUT_PULLUP);  // Someone said this also helped with errant readings...
  pinMode(triggerPin1, OUTPUT);  // Make the triggerPin an OUTPUT
...
void loop()
{
  ...
  int dist1 = 0;
  dist1 = getDistance1();
  Serial.print(dist1); Serial.println("cm");   // took Serial.prints() out of getDistance()
  delay(50);
...
int getDistance() {
  ...
  // Clear the triggerPin by setting it LOW and wait for any pulses to expire:
  digitalWrite(triggerPin1, LOW);      // setting state to low and
  delay(2);                           // waiting 2,000uS (or 686cm (>22ft) to eliminate "echo noise")

  // only grab values under 20ft/610cm (for some reason, 676 is a common return error for ∞ distance)
  while ( (distance1 == 0) || (distance1 > 610) ) {
    // Trigger the sensor by setting the triggerPin high for 10 microseconds:
    digitalWrite(triggerPin1, HIGH);         // start sending the 40kHz wave...
    delayMicroseconds(20);                  // sending for 20uS
    digitalWrite(triggerPin1, LOW);          // stop sending 40kHz wave
    ...
    // Catch funky ∞ distance readings
    watchloop1++;        
    if (watchloop1 > 20){      // If errant "676" readings 20 times
      distance1 = 610;         // set distance to 610cm (20ft) 
      break;                  // and break out of loop (not really needed if forced to 610)
    }
